...
void drawResultsDist2() {
  ...
  const int nRptBin[4] = {4,4,7,7};
  double ratioPtBin[4][8] = {{20,30,40,50,150},         
			     {20,30,40,50,150},
			     {20,30,40,50,60,80,100,150},
                             {20,30,40,50,60,80,100,150}};
  double ratioPtBin1[4][8] = {{20,30,40,50,76},
			      {20,30,40,50,78},
			      {20,30,40,50,60,78,108, 138},
			      {20,30,40,50,60,78,108, 138}};
			      

  /*  const int nRptBin[4] = {7,7,7,7};
  double ratioPtBin[4][8] = {{20,30,40,50,60,80,100,150},
			     {20,30,40,50,60,80,100,150},
			     {20,30,40,50,60,80,100,150},
			     {20,30,40,50,60,80,100,150}};
  
  double ratioPtBin1[4][8] ={{20,30,40,50,60,78,108, 138},
			     {20,30,40,50,60,78,108, 138},
			     {20,30,40,50,60,78,108, 138},
			     {20,30,40,50,60,78,108, 138}};
  */
  ...
  */
  ...
  TCanvas* c5_ratio = new TCanvas("c5_ratio","",1500*0.8,600*0.8);
  makeMultiPanelCanvas(c5_ratio,nPtBin+1, 2, 0.0, 0.0, 0.25, 0.2, 0.02);
  for ( int ipt = 1 ; ipt<=nPtBin  ; ipt++) {
    c5_ratio->cd(ipt+nPtBin+2);
   ...
   hTempPt->DrawCopy();
    ...
    double dx1=0.15;
    if ( ipt == nPtBin )
      drawText(Form("p_{T}^{#gamma} > %dGeV ", (int)ptBin[ipt-1]), 0.10+dx1+0.1,0.85,1,18);
    //  else if ( ipt == 1)
    //  drawText(Form("%dGeV < p_{T}^{#gamma} < %dGeV ", (int)ptBin[ipt-1], (int)ptBin[ipt]), 0.25+dx1,0.85,1,18);
    else
      drawText(Form("%dGeV < p_{T}^{#gamma} < %dGeV ", (int)ptBin[ipt-1], (int)ptBin[ipt]), 0.10+dx1,0.85,1,18);
  }
  ...
  for ( int ibin =1 ; ibin<=nRptBin40GeV ; ibin++)  {
    ...
    jumSun(10,1,150,1);
    double dx1=0.15;
    drawText("p_{T}^{#gamma} > 40GeV ", 0.10+dx1+0.1,0.85,1,18);
