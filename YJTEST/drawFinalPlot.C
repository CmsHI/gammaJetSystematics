...
void SetUncertMultiply(TH1D* h1, float scaleFactor=10.0) {
    int nBins = h1 -> GetNbinsX();
    for(int j=1; j<=nBins; j++){
        float err = h1->GetBinError(j);
        h1->SetBinError(j, err * scaleFactor);
        cout << " bin" << j << " !!  origin err : " << err << ", 10 * err : " << err * scaleFactor << endl;
    }
}
...
void drawFinalPlot() {
    ...
    for ( int ipt = 1 ; ipt <=nPtBin ; ipt++) {
        ...
        SetUncertMultiply(dNdJetPt[1][ipt]);
    }
    ...
    for ( int ipt = 1 ; ipt <=nPtBin ; ipt++) {
        ...
        SetUncertMultiply(dNdJetPt[2][ipt]);
    }
    ...
    for ( int coll = 3 ; coll<=4 ; coll++) { // 3 : centralityBin1(central), 4 : centralityBin2(peripheral)
        for ( int ipt = 1 ; ipt<=nPtBin  ; ipt++) {
            ...
            hTempPt->SetAxisRange(0.0001,2.9,"Y");
